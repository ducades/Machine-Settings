...
static void lcd_advanced_feedrates();
...
static char* lcd_advanced_item(uint8_t nr)
{
    if (nr == 0)
        ...
        strcpy_P(card.longFilename, PSTR("Type Settings"));
    else if (nr == 2)
...
static void lcd_menu_maintenance_advanced()
{
    ...
    if (lcd_lib_button_pressed)
    {
...
#endif
#if TEMP_SENSOR_BED != 0
        else if (IS_SELECTED_SCROLL(3 + EXTRUDERS))                     // Heatup buildplate
        {
            enquecommand_P(PSTR("G28 Z"));
            lcd_change_to_menu(lcd_menu_maintenance_advanced_bed_heatup, 0);
        }
#endif
        else if (IS_SELECTED_SCROLL(3 + BED_MENU_OFFSET + EXTRUDERS))   // Home head
        {
            lcd_lib_beep();
        ...
        else if (IS_SELECTED_SCROLL(5 + BED_MENU_OFFSET + EXTRUDERS))   // Raise bed
        {
            char buffer[32];
            lcd_lib_beep();
            enquecommand_P(PSTR("G28 Z"));
        ...
        else if (IS_SELECTED_SCROLL(6 + BED_MENU_OFFSET + EXTRUDERS))   // Insert material
        {
            char buffer[32];
            enquecommand_P(PSTR("G28 X Y"));
